...
/**
 * The copy and download controls under a table.
 *
 * The confirmation is a toast rather than a note beside the button: these
 * controls sit under tables that scroll inside their own box, so a note here
...
export function tableActions(key, label) {
  return html`<div class="actions actions--table">
    ...
    <button type="button" class="btn btn--small" data-act="csv-table" data-table="${key}">
      ${raw(icon('export'))}Download CSV</button>
  </div>`;
}
