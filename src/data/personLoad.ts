...
export function freeCapacityByPerson({
  ...
  for (const initiative of initiatives) {
    ...
    for (const [phaseId, plan] of Object.entries(initiative.phases ?? {})) {
      ...
      for (const { personId, allocationPct } of plan.allocations) {
        if (!wanted.has(personId) || overlap.length === 0) continue;
  ...
  const teamFtePct = new Map(memberships.filter((m) => m.teamId === teamId && m.active).map((m) => [m.personId, m.teamFtePct]));
  return new Map(
    people.map((person) => {
      ...
      const free = Math.min(
        ...months.map((m) => {
          ...
          return Math.min(person.capacityPct - (cell?.total ?? 0), (teamFtePct.get(person.id) ?? 0) - (cell?.onTeam ?? 0));
        }),
      );
