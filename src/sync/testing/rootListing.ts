 ...
 * only need to be present; `shas` names them when a test also serves the files with versions.
 */
export function rootListing(shas: Record<string, string> = {}): Response {
  const names = ['dataset', 'roles', 'countries', 'teams', 'people', 'memberships'].map((n) => `${n}.json`);
  return new Response(JSON.stringify(names.map((name) => ({ name, path: name, sha: shas[name] ?? `sha-${name}`, type: 'file' }))), { status: 200 });
}
